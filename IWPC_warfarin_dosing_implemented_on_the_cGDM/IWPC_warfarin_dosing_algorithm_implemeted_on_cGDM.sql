...
# CALL Get_Wafarin_Pharmacogenomic_Dosing_Result ( 39, 182, 77, 'NA12283', 'U','Y','Y'); -- Valid case   
# CALL Get_Wafarin_Pharmacogenomic_Dosing_Result ( 44, 165, 60, 'NA11843', 'U','Y','Y'); -- Valid case   
#
...
FROM GENOMIC_ALTERATION GA WHERE HGNC_Gene_Symbol = 'VKORC1' AND dbSNP_ID in ('rs9923231') AND Reference_Allele = 'C' AND Alternative_Allele = 'T' AND BI_Protocol_Identifier = @BI_Protocol_Identifier
UNION
...
SET @VKORC1_Genotype =  (SELECT CONCAT(MAX(CASE WHEN LEFT(Genotype,1) = 1 THEN 'A' ELSE 'G' END) 
,'/', MAX(CASE WHEN RIGHT(Genotype,1) = 1 THEN 'A' ELSE 'G' END) ) 
FROM Wafarin_Related_Variant_Data WHERE BI_Protocol_Identifier = @BI_Protocol_Identifier AND HGNC_Gene_Symbol = 'VKORC1' GROUP BY BI_Protocol_Identifier);
...
FROM Wafarin_Related_Variant_Data WHERE BI_Protocol_Identifier = @BI_Protocol_Identifier AND HGNC_Gene_Symbol = 'CYP2C9' GROUP BY BI_Protocol_Identifier);
...
ELSE     
		...
		INSERT INTO IWPC_coefficient_given_genotype_table VALUES ('VKORC1','G/A',0.8677);
